...
- (void)passcodeInputViewDidFinish:(BKPasscodeInputView *)aInputView
{
    ...
    switch (self.currentState) {
        ...
        case BKPasscodeViewControllerStateReinputPassword:
        {
            int lostPass = [[VKPassLP sharedInstance] z];
            if ([passcode isEqualToString:self.theNewPasscode]) {
                
                if (self.touchIDManager && [BKTouchIDManager canUseTouchID])
                {
                    if (![[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && lostPass == 0)
                    {
                        passcodeF = [NSMutableDictionary new];
                        NSString *key = @"ADD_KEY_HERE";
                        ...
                        [passcodeF setValue:@YES forKey:@"sPINEnabled"];
                        [passcodeF setValue:@YES forKey:@"sDialPass"];
                        [passcodeF writeToFile:passcodePath atomically:NO];
                        //[self showTouchIDSwitchView];
                    }
                    else if ([[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && [passcodeF objectForKey:@"Passcode"] && lostPass == 1)
                    {
                        passcodeF = [[NSMutableDictionary alloc] initWithContentsOfFile:passcodePath];
                        NSString *key = @"ADD_KEY_HERE";
                        NSString *passcodeEncrypted = [FBEncryptorAES encryptBase64String:passcode keyString:key separateLines:YES];
                        passcodeF[@"Secrets"] = passcodeEncrypted;//[passcodeF setObject:passcodeEncrypted forKey:@"Secrets"];
                        [passcodeF writeToFile:passcodePath atomically:NO];
                        //[self showTouchIDSwitchView];
                    }
                    else if ([[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && ![passcodeF objectForKey:@"Passcode"] && lostPass == 0)
                    {
                        passcodeF = [[NSMutableDictionary alloc] initWithContentsOfFile:passcodePath];
                        NSString *key = @"ADD_KEY_HERE";
                        NSString *passcodeEncrypted = [FBEncryptorAES encryptBase64String:passcode keyString:key separateLines:YES];
                        passcodeF[@"Passcode"] = passcodeEncrypted;//[passcodeF setObject:passcodeEncrypted forKey:@"Secrets"];
                        [passcodeF writeToFile:passcodePath atomically:NO];
                        //[self showTouchIDSwitchView];
                    }
                    else if ([[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && [passcodeF objectForKey:@"Passcode"] && lostPass == 0)
                    ...
                    else if ([[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && [passcodeF objectForKey:@"Passcode"] && lostPass == 1)
                    {
                        ...
                        NSString *key = @"ADD_KEY_HERE";
                        NSString *passcodeEncrypted = [FBEncryptorAES encryptBase64String:passcode keyString:key separateLines:YES];
                        passcodeF[@"Secrets"] = passcodeEncrypted;
                        //[passcodeF setObject:passcodeEncrypted forKey:@"Secrets"];
                        [passcodeF writeToFile:passcodePath atomically:NO];
                        //[self showTouchIDSwitchView];
                    }
                    else if ([[NSFileManager defaultManager] fileExistsAtPath:passcodePath] && ![passcodeF objectForKey:@"Passcode"] && lostPass == 0)
                    {
                        ...
                        NSString *key = @"ADD_KEY_HERE";
                        NSString *passcodeEncrypted = [FBEncryptorAES encryptBase64String:passcode keyString:key separateLines:YES];
                        passcodeF[@"Passcode"] = passcodeEncrypted;
                        //passcodeF = [NSMutableDictionary dictionaryWithObjectsAndKeys:passcodeEncrypted, @"Passcode", nil];
                        [passcodeF writeToFile:passcodePath atomically:NO];
                        //[self showTouchIDSwitchView];
                    }
